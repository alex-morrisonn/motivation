...
// Shared Quote structure that works for both the app and widget
struct SharedQuote: Identifiable, Codable, Equatable {
    var id = UUID()
    let text: String
    let author: String
    let category: String
    
    static func == (lhs: SharedQuote, rhs: SharedQuote) -> Bool {
        return lhs.id == rhs.id
    }
}

// All quotes defined in one central location with consolidated categories
struct SharedQuotes {
    static let all: [SharedQuote] = [
        // Success & Achievement
